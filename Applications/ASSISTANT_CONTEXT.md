...
- 07.04.2024 - Обновление моделей данных для продаж:
  - Переработана модель `Sale`:
    - Добавлены поля для клиентской информации (`ClientName`, `ClientPhone`)
    - Добавлены поля для финансовой информации (`TotalAmount`, `DiscountAmount`)
    - Добавлены поля для метаданных (`SaleDate`, `PaymentMethod`, `Comment`)
    - Настроены навигационные свойства с `virtual` для поддержки ленивой загрузки
  - Переработана модель `SaleProduct`:
    - Добавлены поля для информации о продукте (`ProductName`, `ProductCategory`)
    - Добавлены поля для финансовой информации (`ProductPrice`, `DiscountAmount`)
    - Добавлено поле для количества (`Quantity`)
    - Настроены навигационные свойства с `virtual` для поддержки ленивой загрузки
  - Настроены связи между моделями:
    - `Sale` -> `Order` (один к одному)
    - `Sale` -> `SaleProduct` (один ко многим)
    - `SaleProduct` -> `Product` (один к одному)
  - Обновлена конфигурация базы данных:
    - Настроены правильные связи между сущностями
    - Добавлены ограничения на удаление (`DeleteBehavior.Restrict` и `DeleteBehavior.Cascade`)
    - Настроена поддержка ленивой загрузки через `UseLazyLoadingProxies()`
  - Реализована аналитика продаж:
    - Добавлены методы для анализа продаж
    - Реализована статистика по заказам
